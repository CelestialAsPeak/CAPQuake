...
【UI 组件】
- alert_hub       : 预警中枢组件（融合实时最大震度、最新预警、API状态）
...
- earthquake_list : 地震列表面板（滚动列表，支持数据源分组）
...
- ui (UIContext)                     : 聚合所有 UI 组件（字体、按钮、波形等）
...
- alert_hub (AlertHub)               : 预警中枢组件（位于屏幕底部）
- status_reporter (StatusReporter)   : 轻量级状态报告器，供数据源线程更新 API 连接状态
...
1. 模块级公共函数（供其他模块或测试调用）：
   ...
   start_data_fetchers(status_reporter, eew_manager) -> List[Thread]
       启动所有数据源线程，返回线程列表。
...
- 旧的时间显示和预警弹窗组件已完全移除，由 AlertHub 统一接管底部区域。
================================================================================
"""
import os
import json
...
from earthquake_list import EarthquakeListPanel
...
from station.palert_monitor import PAlertMonitor
...
# ========== 今日最大地震持久化配置 ==========
TODAY_MAX_FILE = "today_max_earthquake.json"   # 存储文件（程序根目录）
MAX_SAVE_DAYS = 14                              # 最多保存天数
# ============================================================================
# 轻量级状态报告器（替代旧 TimeDisplay 的状态管理功能）
# ============================================================================
class StatusReporter:
    """
    轻量级状态报告器，仅用于接收数据源连接状态，不涉及任何 UI 绘制。
    提供与 TimeDisplay 兼容的 update_status 接口，供 data_fetchers 调用。
    """
    def __init__(self):
        # 状态字典，可被外部读取（例如 AlertHub 需要获取状态）
        self.source_status = {
            'WOLFX': False,
            'FAN': False,
            'P2P': False,
    ...
    def update_status(self, source: str, is_connected: bool) -> None:
        """更新指定数据源的连接状态（线程安全）"""
        if source in self.source_status:
            self.source_status[source] = is_connected

    def get_status(self, source: str) -> bool:
        """获取指定数据源的当前状态"""
        return self.source_status.get(source, False)
...
# 辅助函数（独立工具）
# ============================================================================
def load_today_max_history():
    """加载历史今日最大地震记录，返回列表（按日期倒序）"""
    if not os.path.exists(TODAY_MAX_FILE):
        return []
    try:
        with open(TODAY_MAX_FILE, 'r', encoding='utf-8') as f:
            data = json.load(f)
        # 过滤超过14天的记录
        cutoff = datetime.now(timezone(timedelta(hours=8))) - timedelta(days=MAX_SAVE_DAYS)
        filtered = []
        for item in data:
            try:
                date = datetime.strptime(item['date'], '%Y-%m-%d').replace(tzinfo=timezone(timedelta(hours=8)))
                if date >= cutoff:
        ...
        print(f"[警告] 加载今日最大地震历史失败: {e}")
        return []

def save_today_max_history(history):
    """保存历史记录到文件"""
    try:
        with open(TODAY_MAX_FILE, 'w', encoding='utf-8') as f:
            json.dump(history, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print(f"[警告] 保存今日最大地震历史失败: {e}")

def get_today_max_from_history(history):
    """从历史记录中获取今天的最大地震"""
    today_str = datetime.now(timezone(timedelta(hours=8))).strftime('%Y-%m-%d')
    for item in history:
        if item.get('date') == today_str:
            return item.get('max_earthquake')
    return None
...
def start_data_fetchers(
    ...
    参数:
        status_reporter: 状态报告器实例，供数据源更新连接状态
    ...
    fetcher = data_fetchers.FanDataFetcher(
        url="wss://ws.fanstudio.tech/all",
        time_display=status_reporter   # 注意：data_fetchers 内部使用 time_display.update_status，这里传入 StatusReporter 兼容
    )
    fetcher.start()
    ...
    if config.P2P_HISTORY_ENABLED:
        p2p_fetcher = data_fetchers.P2PHistoryFetcher(time_display=status_reporter)
        p2p_fetcher.start()
...
class CAPQuakeApp:
    ...
    属性:
        ...
        map_surface: 地图绘制表面
        alert_hub: 预警中枢组件
        status_reporter: 状态报告器（供数据源更新连接状态）
    """
    ...
    def __init__(self) -> None:
        ...
        self.alert_hub: Optional[AlertHub] = None
        self.status_reporter = StatusReporter()   # 轻量级状态报告器
        ...
        # 初始化 P-Alert 监控器（CSV 路径请改为你的实际路径）
        palert_csv = r"C:\Users\ZhuanZ.DESKTOP-PH97BKO\Desktop\CAPQuake\StationJson\PAlertStation.csv"
        self.palert_monitor = PAlertMonitor(palert_csv)
        self.palert_monitor.start()
    ...
    def _init_components(self) -> None:
        ...
        self.alert_hub = AlertHub(width, height)
        bottom_reserve = AlertHub.get_bottom_occupied_height()

        # 创建地震列表面板，传入底部预留空间
        self.earthquake_list_panel = EarthquakeListPanel(width, height, bottom_reserve=bottom_reserve)

        # 加载今日最大地震历史
        self.today_max_history = load_today_max_history()
        today_max_data = get_today_max_from_history(self.today_max_history)
        if today_max_data and self.alert_hub:
            self.alert_hub.set_today_max_earthquake(today_max_data)
    ...
    def _build_ui(self, fonts: FontContext) -> UIContext:
        ...
        max_intensity_display = MaxIntensityDisplay(None, None)
    ...
    def _start_data_fetchers(self) -> None:
        ...
        fetchers = start_data_fetchers(self.status_reporter, self.eew_manager)
    ...
    def _handle_events(self) -> None:
        ...
        for event in pygame.event.get():
            ...
            # 鼠标滚轮事件（缩放地图）
                ...
                # 检查鼠标是否在地震列表面板内
                panel = getattr(self, 'earthquake_list_panel', None)
                mouse_pos = pygame.mouse.get_pos()
                if panel and panel.get_rect().collidepoint(mouse_pos):
                    # 在面板内：只滚动列表
    ...
    def _handle_resize(self, new_width: int, new_height: int) -> None:
        ...
        # 更新预警中枢和地震列表
        if hasattr(self, 'alert_hub') and self.alert_hub:
            self.alert_hub.on_window_resize(new_width, new_height)
            bottom_reserve = AlertHub.get_bottom_occupied_height()
            self.earthquake_list_panel.update(
    ...
    def _update_components(self) -> None:
        ...
        # 更新地震列表面板
        if hasattr(self, 'earthquake_list_panel') and self.earthquake_list_panel:
            width, height = self.screen.get_size()
            bottom_reserve = AlertHub.get_bottom_occupied_height()
            self.earthquake_list_panel.update(
                state.earthquake_events,
        ...
        # ========== 更新今日最大地震（从 earthquake_events 中计算并持久化） ==========
        if state.earthquake_events:
            now_cst = datetime.now(timezone(timedelta(hours=8)))
            today_str = now_cst.strftime('%Y-%m-%d')
            today_events = []
            for event in state.earthquake_events:
                main = event['main']
                time_str = main.get('time', '')
                if not time_str:
                    continue
                try:
                    ...
                    if event_time.strftime('%Y-%m-%d') == today_str:
                        mag = main.get('mag')
                        if mag is not None:
                            today_events.append((mag, main))
                except:
                    continue
            if today_events:
                # 按震级降序排序，取第一个
                today_events.sort(key=lambda x: x[0] if x[0] is not None else -1, reverse=True)
                max_mag, max_main = today_events[0]
                new_data = {
                    'place': max_main.get('place', ''),
                    'time': max_main.get('time', ''),
                    'mag': max_main.get('mag'),
                    'depth': max_main.get('depth'),
                    'intensity': max_main.get('intensity'),
                    'source': max_main.get('source', ''),
                }
                # 更新历史记录（去重，保留最近14天）
                existing_idx = None
                for i, item in enumerate(self.today_max_history):
                    if item['date'] == today_str:
                        existing_idx = i
                        break
                updated = False
                if existing_idx is not None:
                    old_mag = self.today_max_history[existing_idx]['max_earthquake'].get('mag', -1)
                    if max_mag > old_mag:
                        self.today_max_history[existing_idx]['max_earthquake'] = new_data
                        updated = True
                else:
                    self.today_max_history.append({'date': today_str, 'max_earthquake': new_data})
                    updated = True
                if updated:
                    # 按日期倒序排序，保留最近 MAX_SAVE_DAYS 天
                    self.today_max_history.sort(key=lambda x: x['date'], reverse=True)
                    if len(self.today_max_history) > MAX_SAVE_DAYS:
                        self.today_max_history = self.today_max_history[:MAX_SAVE_DAYS]
                    save_today_max_history(self.today_max_history)
                    if self.alert_hub:
                        self.alert_hub.set_today_max_earthquake(new_data)
            # 如果今天没有地震，可以选择不清空已有显示（保持之前的数据）

        # ========== 更新预警中枢组件 ==========
        # 1. 获取活跃预警列表（未过期的预警，可能多个）
        active_warnings = []
        active_eews = self.eew_manager.get_all_active()
        # 按烈度降序排序（烈度高的优先显示）
        active_eews.sort(key=lambda x: x.get('intensity', 0) if x.get('intensity') is not None else -1, reverse=True)
        for eew in active_eews:
            active_warnings.append({
                'source': eew.get('source', ''),
                'event_id': eew.get('event_id', ''),
                'time': eew.get('time', ''),
                'location': eew.get('location', ''),
                'mag': eew.get('mag', '?'),
                'depth': eew.get('depth', '?'),
                'intensity': eew.get('intensity', None),
                'report_no': eew.get('report_no', 1),
            })

        # 2. 获取历史预警（最近收到的预警，最多6条）
        history_warnings = []
        recent_eews = self.eew_manager.get_recent_eews()
        for eew in recent_eews[:6]:
        ...
        try:
            if self.monitors.snet_monitor:
                stations = self.monitors.snet_monitor.get_stations()
                if stations:
                    max_intensities['snet'] = max(s[2] for s in stations)
        except Exception as e:
            print(f"[警告] S-net 数据获取失败: {e}")

        # KMA
        try:
            if self.monitors.kma_monitor:
                stations = self.monitors.kma_monitor.get_stations()
                if stations:
                    max_intensities['kma'] = max(s[2] for s in stations)
        except Exception as e:
            print(f"[警告] KMA 数据获取失败: {e}")

        # NIED (Yahoo) - 使用官方映射表
        try:
            if self.monitors.yahoo_monitor:
                stations = self.monitors.yahoo_monitor.get_current_stations()
                if stations:
                    # 找到最大 yahoo_int 对应的站
                    max_yahoo = max(stations, key=lambda x: x[2])[2]
                    from station.yahoo_kyoshin_monitor import yahoo_intensity_to_jma_text
                    from utils import jma_intensity_text_to_number
                    jma_text = yahoo_intensity_to_jma_text(max_yahoo)
                    jma_value = jma_intensity_text_to_number(jma_text)
                    max_intensities['nied'] = jma_value
        except Exception as e:
            print(f"[警告] NIED 数据获取失败: {e}")

        # 全球 PGA
        try:
            if self.monitors.pga_monitor:
                station_data = self.monitors.pga_monitor.get_station_data()
                if station_data:
                    max_intensities['pga'] = max(data['intensity'] for data in station_data.values())
        except Exception as e:
            print(f"[警告] PGA 数据获取失败: {e}")

        # P-Alert 实时最大震度（从 PAlertMonitor 获取）
        try:
            if hasattr(self, 'palert_monitor') and self.palert_monitor:
                stations = self.palert_monitor.get_stations()
                if stations:
                    max_shindo = -1.0
                    for lat, lon, pga, intensity_csis, name in stations:
                        if pga > 0:
                            # 智能单位转换：如果 PGA < 0.5，视为 m/s²，转为 gal
                            if pga < 0.5:
                                pga_gal = pga * 100
                            else:
                                pga_gal = pga
                            # Excel 公式：震度 = 2 * log10(PGA_gal) + 0.45
                            shindo = 2 * math.log10(pga_gal) + 0.45
                            # 限制 JMA 震度范围 0.0 ~ 7.0
                            shindo = max(0.0, min(7.0, shindo))
                            if shindo > max_shindo:
                                max_shindo = shindo
                    if max_shindo >= 0:
                        max_intensities['jma_alert'] = max_shindo
        except Exception as e:
            print(f"[警告] P-Alert 数据获取失败: {e}")

        # 4. 获取 API 状态（从 status_reporter 获取）
        api_status = {
            'WOLFX': self.status_reporter.get_status('WOLFX'),
            'FAN': self.status_reporter.get_status('FAN'),
            'P2P': self.status_reporter.get_status('P2P'),
        }

        # 5. 更新 AlertHub
        if self.alert_hub:
            self.alert_hub.update(
                history_warnings=history_warnings,
                max_intensities=max_intensities,
                api_status=api_status,
                active_warnings=active_warnings   # 注意参数名是复数
            )
    ...
    def _render(self) -> None:
        ...
        # 绘制 P-Alert 测站
        if hasattr(self, 'palert_monitor') and self.palert_monitor:
            draw._draw_palert_stations(self.map_surface, width, height, self.palert_monitor)
        ...
        draw.draw_ui_overlay(
            ...
            earthquake_list_panel=self.earthquake_list_panel
        )

        # 绘制预警中枢组件（位于底部）
        if self.alert_hub:
            self.alert_hub.reset_bottom_alignment()
            self.alert_hub.draw(self.screen)
